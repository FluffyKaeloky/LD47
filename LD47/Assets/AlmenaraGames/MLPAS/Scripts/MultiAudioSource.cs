...
namespace AlmenaraGames{
	...
	void Update () {
		...
		if (Object.ReferenceEquals(audioObject, null)) {
           ...
           /* if ((play || playing))
			...
			if (onFocus && !MultiAudioManager.Paused && !globalPaused && !play && playing && (true && !loop && !audioSource.isPlaying || pitch<0 && loop && audioSource.time <= 0.01f || pitch>=0 && loop && audioSource.time >= audioSource.clip.length - 0.01f || true && loop && !audioSource.isPlaying)) {
           ...
           if (dopplerLevel > 0 && (play || playing))
            {
                ...
                DopplerFactor *= MultiPoolAudioSystem.audioManager.config.dopplerFactor;
	...
	void SetParameters(bool onUpdate=false)
	{
		...
		if (OverrideValues && overrideMixerGroup && mixerGroupOverride!=null)
			...
			if (!Object.ReferenceEquals(audioObject.mixerGroup,null)) {
...
/*
	...
	public class MultiAudioSourceEditor: Editor 
	{
		...
		public override void OnInspectorGUI()
		{
            ...
            EditorGUILayout.Space();
            Tools.MLPASExtensionMethods.IntField(defaultChannelProp, new GUIContent("Channel", "To play the Audio Object in a certain channel, use a channel greater than -1"), -1, int.MaxValue);
