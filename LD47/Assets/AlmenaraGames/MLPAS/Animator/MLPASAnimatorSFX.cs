       ...
       // public int layerSync;
            ...
            for (int i = 0; i < stateSfxs.Count; i++)
            {
                ...
                if (stateSFX.playMode == MLPASAnimatorPlayMode.OnUpdate && !onStateMachine)
                {
                   ...
                   if (stateSFX.lastNormalizedTime > stateInfo.normalizedTime)
                    ...
                    float sfxPlayTime = Mathf.Clamp((stateSFX.normalizedPlayTime), 0.01f, 0.99f) + stateSFX.curLoop;
                    ...
                    if (!stateSFX.playOnlyOnce || playOnceTest)
                    {
                        ...
                        if (sfxPlayTime >= lastNormalizedTime && sfxPlayTime < currentNormalizedTime && (sfxPlayTime - lastNormalizedTime <= Time.deltaTime * 10))
                        {
        ...
        override public void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            for (int i = 0; i < stateSfxs.Count; i++)
            {
                ...
                if (!stateSFX.sfxPlayed && stateSFX.conditionEvaluated && stateSFX.playMode == MLPASAnimatorPlayMode.OnTransitionExit)
                {
                    PlaySFX(stateInfo, stateSFX, GetBlendValue(stateSFX, animator), animator);
                    stateSfxs[i].sfxPlayed = true;
                }
           ...
           /* if (onSyncedLayer)
            ...
            if (!isBlendTree)
            {
                ...
                if (followPos)
                {
                    ...
                    if (stateSfx.useIdentifier)
                        MultiAudioManager.PlayAudioObjectByIdentifier(stateSfx.audioObjectIdentifier, stateSfx.useChannel ? stateSfx.channel : -1, usingDifferentPlayPos ? sfxValues.playPosition : trf);
                    else
                        MultiAudioManager.PlayAudioObject(stateSfx.audioObject, stateSfx.useChannel ? stateSfx.channel : -1, usingDifferentPlayPos ? sfxValues.playPosition : trf);
                ...
                else
                {
                    ...
                    if (stateSfx.useIdentifier)
                        MultiAudioManager.PlayAudioObjectByIdentifier(stateSfx.audioObjectIdentifier, stateSfx.useChannel ? stateSfx.channel : -1, usingDifferentPlayPos ? sfxValues.playPosition.position : trf.position);
                    else
                        MultiAudioManager.PlayAudioObject(stateSfx.audioObject, stateSfx.useChannel ? stateSfx.channel : -1, usingDifferentPlayPos ? sfxValues.playPosition.position : trf.position);
        ...
        public class MLPASAnimatorSFXEditor : Editor
        {
                        ...
                        else
                        {
                            ...
                            if (showBlendtree)
                            {
                                ...
                                for (int iB = 0; iB < blendTree.children.Length; iB++)
                                {
                                    ...
                                    if (GUILayout.Button(blendTreeItemName, itemStyle))
                                    {
            ...
            void DuplicateStateSFX(MLPASAnimatorSFX.StateSFX original, ref MLPASAnimatorSFX.StateSFX copy)
            {
                ...
                if (sameName)
                {
                    ...
                    {
                        copy.methodName = original.methodName + " Copy";
                    }
                    else
                    {
                        bool valueSet = false;
